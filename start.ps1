...
try {
    ...
    Write-Host "❌ Docker is not available. Please install Docker Desktop or Docker CLI." -ForegroundColor Red
    exit 1
}
...
if (-not (Test-Path '.env')) {
    if (Test-Path '.env.example') {
        ...
        Write-Host "⚠️  No .env or .env.example found. Proceeding with defaults..." -ForegroundColor Yellow
    }
}

# Start containers
Write-Host "▶️  Running docker compose up -d..." -ForegroundColor Cyan
docker compose up -d
$composeResult = $?
...
if ($attempts -eq $maxAttempts) {
    Write-Host "⚠️  PostgreSQL did not respond after $maxAttempts seconds, but proceeding..." -ForegroundColor Yellow
}
