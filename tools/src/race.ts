...
// httpRequest по контракту (lib/http.ts) не бросает, поэтому мёртвая заглушка молча
// оставляла бы режим normal (STUB_FAIL_RATE=0.2) и гонка порождала бы отказы, которые
// оператор прочитает как баги продукта
...
function runHttpChecks(
  results: { status: number; body: IWebhookResultBody | null }[],
): ICheckRow[] {
  ...
  rows.push(
    allHttp200
      ? pass('http-200', 'все ответы вебхука — HTTP 200')
      : fail('http-200', 'встретился ответ не 200'),
  );
  ...
  rows.push(
    applied === 1
      ? pass('exactly-one-applied', `applied=${applied}`)
      : fail('exactly-one-applied', `applied=${applied}`),
  );
  ...
  rows.push(
    unexpected === 0
      ? pass('no-unexpected-results', 'нет duplicate/conflict/rejected_amount/orphan')
  ...
  rows.push(
    otherAllowed
      ? pass(
          'remaining-results-known',
          'все результаты — applied/ignored_stale/ignored_already_paid',
        )
...
async function runDeliveryChecks(
  options: IRaceCliOptions,
  extId: string,
): Promise<{ rows: ICheckRow[]; delivery: IOrderDetailResponse | null }> {
  const rows: ICheckRow[] = [];
  const lookup = await httpGet<IOrderDetailResponse>(
    `${options.apiBaseUrl}${ORDERS_PATH}/${extId}`,
    options.timeoutMs,
  );

  rows.push(
    lookup.ok && lookup.body !== null
      ? pass('order-lookup', 'GET /orders/:orderId вернул 200')
      : fail('order-lookup', `status=${lookup.status}`),
  );
  ...
  rows.push(
    reachedTerminal
      ? pass('order-terminal', `status=${last?.status ?? 'unknown'}`)
      : fail('order-terminal', ORDER_NOT_DELIVERED_MESSAGE),
  );
...
async function runDbChecks(
  ...
  const eventsTotal = await client.query<{ count: number }>(COUNT_PAYMENT_EVENTS_BY_ORDER_SQL, [
    extId,
  ]);
  ...
  rows.push(
    total === options.count
      ? pass('payment-events-total', `count=${total}`)
      : fail('payment-events-total', `count=${total}, ожидалось ${options.count}`),
  );

  const unexpectedStates = await client.query<{ count: number }>(
    COUNT_UNEXPECTED_PAYMENT_EVENTS_SQL,
    [extId],
  );
  const eventsApplied = await client.query<{ count: number }>(COUNT_APPLIED_PAYMENT_EVENTS_SQL, [
    extId,
  ]);
  ...
  rows.push(
    jobs === 1
      ? pass('deliver-order-job-count', `jobs=${jobs}`)
      : fail('deliver-order-job-count', `jobs=${jobs}, ожидалось 1`),
  );

  const jobStateRow = await client.query<{ state: string }>(SELECT_DELIVER_ORDER_JOB_STATE_SQL, [
    dedupeKey,
  ]);
  ...
  rows.push(
    jobState === JOB_STATE_DONE
      ? pass('deliver-order-job-done', `state=${jobState}`)
      : fail('deliver-order-job-done', `state=${String(jobState)}`),
  );

  const issued = await client.query<{ count: number }>(COUNT_ISSUED_DELIVERIES_BY_ORDER_ID_SQL, [
    orderId,
  ]);
  ...
  rows.push(
    issuedCount === 1
      ? pass('issued-deliveries-count', `count=${issuedCount}`)
      : fail('issued-deliveries-count', `count=${issuedCount}, ожидалось 1`),
  );
  ...
  rows.push(
    txnCount === 2
      ? pass('ledger-txns-count', `count=${txnCount}`)
      : fail('ledger-txns-count', `count=${txnCount}, ожидалось 2`),
  );

  const entries = await client.query<{ count: number }>(COUNT_LEDGER_ENTRIES_BY_ORDER_ID_SQL, [
    orderId,
  ]);
  ...
  rows.push(
    entryCount === 4
      ? pass('ledger-entries-count', `count=${entryCount}`)
      : fail('ledger-entries-count', `count=${entryCount}, ожидалось 4`),
  );

  const balance = await client.query<{ sum: string }>(SUM_SIGNED_MINOR_BY_ORDER_ID_SQL, [orderId]);
  const sum =
    balance.rows[0] === undefined ? -1 : toSafeInt(balance.rows[0].sum, 'ledger-balanced.sum');

  rows.push(
    sum === 0
      ? pass('ledger-balanced', `sum=${sum}`)
      : fail('ledger-balanced', `sum=${sum}, ожидалось 0`),
  );

  const cash = await client.query<{ count: number; sum: string }>(
    CASH_DEBIT_SUMMARY_BY_ORDER_ID_SQL,
    [orderId],
  );
  const cashCount = cash.rows[0]?.count ?? 0;
  const cashSum = cash.rows[0] === undefined ? -1 : toSafeInt(cash.rows[0].sum, 'cash-debit.sum');
  const modeRow = await client.query<{ fulfillment_mode: string }>(
    SELECT_FULFILLMENT_MODE_BY_ORDER_ID_SQL,
    [orderId],
  );
  ...
  if (mode === FULFILLMENT_MODE.POOL) {
    ...
    const succeeded = await client.query<{ count: number }>(
      COUNT_SUCCEEDED_DELIVERY_ATTEMPTS_BY_ORDER_ID_SQL,
      [orderId],
    );
  ...
  rows.push(
    cashCount === 1 && fulfilmentOk
      ...
      : fail(
          'cash-debit-and-fulfilment',
          `cash_debit=${cashCount}/${cashSum}, ${fulfilmentDetail}`,
        ),
  );
...
async function main(): Promise<void> {
  ...
  // управление заглушкой проверяется до создания заказа: иначе мёртвая заглушка оставляет
  // за каждым запуском неоплаченный заказ-мусор
  if (options.resetStubs) {
    await requireStubControl(
      `${options.supplierABaseUrl}${CONTROL_RESET_PATH}`,
      {},
      options.timeoutMs,
    );
  }
  ...
  if (options.useStubControl) {
    await requireStubControl(
      `${options.supplierABaseUrl}${CONTROL_SCENARIO_PATH}`,
      { mode: SCENARIO_MODE_OK },
      options.timeoutMs,
    );
  }

  try {
    // resolveTarget обязан быть внутри try: его падение (например, опечатка в --order)
    // иначе оставило бы заглушку A навсегда в режиме ok и убрало бы инъекцию отказов
    // у всех последующих прогонов на этом стенде
    const target = await resolveTarget(options);

    console.log(
      `Гонка против заказа ${target.extId}: ${options.count} параллельных вебхуков, amount=${target.amountMajor} ${target.currency}`,
    );

    const events = buildRaceEvents(target, options.count, Date.now());
    const results = await Promise.all(
      events.map((event) =>
        httpPost<IWebhookResultBody>(
    ...
    const verdict = printCheckTable([...httpRows, ...deliveryRows, ...dbRows]);

    process.exitCode = EXIT_CODE[verdict];
  } finally {
    // восстановление режима заглушки — уборка, а не проверка: её сбой не должен
    // превращать успешный прогон в exit 1 через main().catch
    if (options.useStubControl) {
      try {
        await requireStubControl(
          `${options.supplierABaseUrl}${CONTROL_SCENARIO_PATH}`,
          { mode: SCENARIO_MODE_NORMAL },
          options.timeoutMs,
        );
      } catch (error: unknown) {
        console.error(
