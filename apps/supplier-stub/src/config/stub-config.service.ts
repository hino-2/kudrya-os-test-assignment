...
function parseSupplierId(raw: string | undefined, issues: IEnvIssue[]): SupplierId {
  ...
  if (!SUPPLIER_IDS.includes(value as SupplierId)) {
    ...
    return STUB_CONFIG_ENV_DEFAULTS.SUPPLIER_ID;
  }
...
export class StubConfigService {
  ...
  constructor() {
    ...
    const inventorySize = parseIntEnv(
      'STUB_INVENTORY_SIZE',
      env.STUB_INVENTORY_SIZE,
      STUB_CONFIG_ENV_DEFAULTS.STUB_INVENTORY_SIZE,
      issues,
    );
    const failRate = parseRateEnv(
      'STUB_FAIL_RATE',
      env.STUB_FAIL_RATE,
      STUB_CONFIG_ENV_DEFAULTS.STUB_FAIL_RATE,
      issues,
    );
    const timeoutRate = parseRateEnv(
      'STUB_TIMEOUT_RATE',
      env.STUB_TIMEOUT_RATE,
      STUB_CONFIG_ENV_DEFAULTS.STUB_TIMEOUT_RATE,
      issues,
    );
    const slowRate = parseRateEnv(
      'STUB_SLOW_RATE',
      env.STUB_SLOW_RATE,
      STUB_CONFIG_ENV_DEFAULTS.STUB_SLOW_RATE,
      issues,
    );
    const latencyMinMs = parseIntEnv(
      'STUB_LATENCY_MS_MIN',
      env.STUB_LATENCY_MS_MIN,
      STUB_CONFIG_ENV_DEFAULTS.STUB_LATENCY_MS_MIN,
      issues,
    );
    const latencyMaxMs = parseIntEnv(
      'STUB_LATENCY_MS_MAX',
      env.STUB_LATENCY_MS_MAX,
      STUB_CONFIG_ENV_DEFAULTS.STUB_LATENCY_MS_MAX,
      issues,
    );
    const hangMs = parseIntEnv(
      'STUB_HANG_MS',
      env.STUB_HANG_MS,
      STUB_CONFIG_ENV_DEFAULTS.STUB_HANG_MS,
      issues,
    );
    const controlEnabled = parseBoolEnv(
      'STUB_CONTROL_ENABLED',
      env.STUB_CONTROL_ENABLED,
      STUB_CONFIG_ENV_DEFAULTS.STUB_CONTROL_ENABLED,
      issues,
    );
    const logLevel = parseEnumEnv(
      'LOG_LEVEL',
      env.LOG_LEVEL,
      LOG_LEVELS,
      STUB_CONFIG_ENV_DEFAULTS.LOG_LEVEL,
      issues,
    );
    const logFormat = parseEnumEnv(
      'LOG_FORMAT',
      env.LOG_FORMAT,
      LOG_FORMATS,
      STUB_CONFIG_ENV_DEFAULTS.LOG_FORMAT,
      issues,
    );
    ...
    if (latencyMinMs > latencyMaxMs) {
      issues.push({
        name: 'STUB_LATENCY_MS_MIN',
        reason: 'STUB_LATENCY_MS_MIN не может превышать STUB_LATENCY_MS_MAX',
      });
    }
