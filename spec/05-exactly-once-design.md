...
| **`paid` after `payment_failed`** | **Revised: `kind: 'apply' → paid`.** The event goes through the amount guard and the staleness guard first, so it can only apply when it is a genuinely *newer* provider event carrying the right amount — i.e. a second capture attempt that succeeded. The transition posts `payment_captured` and enqueues delivery like any other `paid`, and logs at `warn`, because it is the one transition that leaves a status the assignment calls final. | Originally this was `conflict` "honouring the spec's *final*", with `POST /admin/orders/:id/force-paid` as the audited recovery path. That path was never built, and neither was `GET /reconciliation/payment-conflicts` — so the real behaviour was: provider captured the money, we posted no `payment_captured`, delivered nothing, and offered no way to even *find* the order. Keeping money without goods is the outcome the original justification itself called unacceptable. Determinism is not lost: ordering is decided by the provider's `occurred_at` through the staleness guard, not by arrival order. Consequence carried through to the contract: `payment_failed` is no longer reported as `terminal: true` (see §4) — a flag that tells a client to stop polling must not sit on a status the provider can still lift. |
